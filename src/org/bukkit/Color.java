...
public final class Color implements ConfigurationSerializable {
    ...
    public static Color fromRGB(int rgb) throws IllegalArgumentException {
        isTrue((rgb >> 24) == 0, "Extrenuous data in: ", rgb);
    ...
    public static Color fromBGR(int bgr) throws IllegalArgumentException {
        isTrue((bgr >> 24) == 0, "Extrenuous data in: ", bgr);
        return fromBGR(bgr >> 16 & BIT_MASK, bgr >> 8 & BIT_MASK, bgr >> 0 & BIT_MASK);
    }

    private Color(int red, int green, int blue) {
        isTrue(red >= 0 && red <= BIT_MASK, "Red is not between 0-255: ", red);
        isTrue(green >= 0 && green <= BIT_MASK, "Green is not between 0-255: ", green);
        isTrue(blue >= 0 && blue <= BIT_MASK, "Blue is not between 0-255: ", blue);
    ...
    public Color mixDyes(DyeColor... colors) {
        noNullElements(colors, "Colors cannot be null");
    ...
    public Color mixColors(Color... colors) {
        ...
        int totalMax = max(max(totalRed, totalGreen), totalBlue);
        for (Color color : colors) {
            totalRed += color.getRed();
            totalGreen += color.getGreen();
            totalBlue += color.getBlue();
            totalMax += max(max(color.getRed(), color.getGreen()), color.getBlue());
        }
        ...
        float maximumOfAverages = max(max(averageRed, averageGreen), averageBlue);
    ...
    public String toString() {
        return "Color:[rgb0x" + toHexString(getRed()).toUpperCase() + toHexString(getGreen()).toUpperCase() + toHexString(getBlue()).toUpperCase() + "]";
    }
}
